...
- Breaking a honeypot block earns points (`offensePointMap`, default 1 per
  block). At `offensePoints` total, the player's breaks and placements are
...
Catches are logged to the console and, with `logToFile: true`, appended to
`plugins/Honeypot/honeypot.log`.
